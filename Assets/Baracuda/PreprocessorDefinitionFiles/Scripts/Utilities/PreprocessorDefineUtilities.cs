...
namespace Baracuda.PreprocessorDefinitionFiles.Scripts.Utilities
{
    ...
    public static class PreprocessorDefineUtilities
    {
        ...
        /// </summary>
        public static IEnumerable<string> VersionDefines => _versionDefines ?? CreateVersionDefines();

        
        /// <summary>
        /// Backend field
        /// </summary>
        private static List<string> _versionDefines = null;
        private static List<string> CreateVersionDefines()
        {
            if (_versionDefines != null) throw new InvalidOperationException(
                $"Only allowed to call {nameof(CreateVersionDefines)} when {nameof(_versionDefines)} is null or empty!");
            
            _versionDefines = new List<string>();
            var splitVersion = Application.unityVersion.Split('.');
            _versionDefines.Add($"UNITY_{splitVersion[0]}"); // eg UNITY_2021
            _versionDefines.Add($"UNITY_{splitVersion[0]}_{splitVersion[1]}"); // eg UNITY_2021_1
            _versionDefines.Add($"UNITY_{splitVersion[0]}_{splitVersion[1]}_OR_NEWER"); // eg UNITY_2021_1_OR_NEWER

            return _versionDefines;
        }
        ...
        private static bool _isProSkin = true;
        
        private static string ColorElevated 
            => _isProSkin? "<color=#75e09c>" : "<color=#1e5438>";
        
        private static string ColorAdded 
            => _isProSkin? "<color=#4babeb>" : "<color=#204c69>";
        
        private static string ColorRemoved 
            => _isProSkin? "<color=#eb4b6b>" : "<color=#99183d>";
